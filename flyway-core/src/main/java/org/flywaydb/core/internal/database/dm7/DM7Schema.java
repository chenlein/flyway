...
import static org.flywaydb.core.internal.database.dm7.DM7Schema.ObjectType.*;
 ...
 * @Description:
 */
public class DM7Schema extends Schema<DM7Database> {

    private static final Log LOG = LogFactory.getLog(DM7Schema.class);
     ...
     */
    public DM7Schema(JdbcTemplate jdbcTemplate, DM7Database database, String name) {
    ...
    protected Table[] doAllTables() throws SQLException {
        ...
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new DM7Table(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new DM7Table(jdbcTemplate, database, this, tableName);
    }

    public enum ObjectType {
        TABLE("TABLE", "CASCADE CONSTRAINTS PURGE") {
            @Override
            public List<String> getObjectNames(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema) throws SQLException {
        ...
        QUEUE_TABLE("QUEUE TABLE") {
            ...
            @Override
            public String generateDropStatement(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema, String objectName) {
        ...
        MINING_MODEL("MINING MODEL") {
            ...
            @Override
            public String generateDropStatement(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema, String objectName) throws SQLException {
        ...
        DATABASE_LINK("DATABASE LINK") {
            @Override
            public void dropObjects(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema) throws SQLException {
         ...
         */
        public static Set<String> getObjectTypeNames(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema) throws SQLException {
            boolean xmlDbAvailable = database.isXmlDbAvailable();
            ...
            String query =
                    ...
                    "SELECT DISTINCT OBJECT_TYPE FROM " + database.dbaOrAll("OBJECTS") + " WHERE OWNER = ? AND OBJECT_TYPE != 'SCH'" +
                            // Materialized view logs.
...
//                            "SELECT * FROM ALL_SCHEDULER_CREDENTIALS WHERE OWNER = ?) "
         ...
         * @throws SQLException if retrieving of object types failed.
         */
        public static boolean supportedTypesExist(JdbcTemplate jdbcTemplate, DM7Database database, DM7Schema schema) throws SQLException {
